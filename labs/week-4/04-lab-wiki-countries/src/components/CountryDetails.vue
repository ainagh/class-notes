...
<template>
  ...
  <!-- https://flagcdn.com/w320/ARM.png  -->
  <img :src="`https://flagcdn.com/w320/${countryInfo.alpha2Code.toLowerCase()}.png`" :alt="`${countryInfo.alpha2Code}`" />
  ...
  <li class="list-group-item">
    <p>Borders</p>
    <p v-if="countryInfo.borders.length === 0"    >Este país no tiene una frontera, es una isla</p>
    <ul v-for="(borderCountry, index) in countryInfo.borders" :key="index">
...
// data property to will be used to store info from the API and then push. la fem nula pq guardi la info
